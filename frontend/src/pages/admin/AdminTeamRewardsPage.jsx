...
export default function AdminTeamRewardsPage() {
  const queryClient = useQueryClient();
  const toast = useToast();

  const { data: requests, isLoading } = useQuery({
    queryKey: ['adminTeamRewards'],
    queryFn: async () => (await api.get('/admin/team-rewards')).data,
  });

  const actionMutation = useMutation({
    mutationFn: async ({ id, action }) => (await api.post(`/admin/team-rewards/${id}/${action}`)).data,
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries(['adminTeamRewards']);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-serif text-white">Team Reward Requests</h1>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-gray-900 border-b border-gray-800 text-gray-400">
            <tr>
              ...
              <th className="p-3">Level Requested</th>
          ...
          <tbody className="divide-y divide-gray-800">
            ...
            {requests?.map(r => (
              <tr key={r.id} className="hover:bg-gray-800/50">
                ...
                <td className="p-3 font-bold text-sky-400">Level {r.level}</td>
                ...
                <td className="p-3 text-right space-x-2">
                  {r.status === 'pending' && (
                    <>
                      <button 
                        onClick={() => {
                          if(window.confirm('Approve this reward?')) {
