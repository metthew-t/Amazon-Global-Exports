...
router.post('/', protect, async (req, res) => {
  try {
    ...
    if (!parsedAmount || parsedAmount < minWithdrawal) return res.status(400).json({ message: `Minimum withdrawal is ${minWithdrawal} ETB` });

    const quickAmountsStr = s['withdrawal_quick_amounts'] || '500,1500,6000,15000,45000,100000';
    const quickAmounts = quickAmountsStr.split(',').map(n => parseInt(n.trim())).filter(n => !isNaN(n) && n > 0);
    if (quickAmounts.length > 0 && !quickAmounts.includes(parsedAmount)) {
    ...
    const id = await db.transaction(async (txDb) => {
      await txDb.prepare('UPDATE users SET balance = balance - ? WHERE id = ?').run(parsedAmount, req.user.id);
      const newId = uuidv4();
      await txDb.prepare(
        `INSERT INTO withdrawals (id, user_id, bank_type, account_number, account_name, amount) VALUES (?, ?, ?, ?, ?, ?)`
      ).run(newId, req.user.id, user.bank_type, user.account_number, user.account_name, parsedAmount);
      return newId;
    });
